...
<body>
    ...
    <?php
        ...
        if (isset($_POST['add_rechnung'])) {
            ...
            $final_Betrag = str_replace('.- €','', $betrag);
            ...
            $Hinzufügen->bindParam(":RE_Nummer", $_POST["RE_Nummer"]);
            $Hinzufügen->bindParam(":Zahlungsfrist", $_POST["Zahlungsfrist"]);
            $Hinzufügen->bindParam(":Betrag", $final_Betrag);
        ...
        <table style="width: 98.5%;"> 
                ...
                <form action="<?= $_SERVER['PHP_SELF']?>" method="POST">
                        <td>
                            ...
                            <td> <input name="Bestellnummer" type="text" class="edit"></td>
    ...
    <div><br>
            <table style="width: 100%;"> 
                    ...
                    <th></th>
                <?php 
                    $teilnehmer = $conn->prepare("SELECT *, rechnung.geloescht AS del FROM rechnung LEFT JOIN teilnehmer ON teilnehmer.ID_Teilnehmer = rechnung.ID_Teilnehmer WHERE rechnung.geloescht!=1");
                    $teilnehmer->execute();
                    
                    while($row = $teilnehmer->fetch(PDO::FETCH_ASSOC)) {
                        if($row["del"] == 0) {
                        ?>
