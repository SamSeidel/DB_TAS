...
--
-- Host: 127.0.0.1
-- Erstellungszeit: 10. Jan 2024 um 14:18
...
(1, 'Herr', 'Michael', 'Schmidt', '1980-05-15', '123-456-7890', 'DE123456789', 'SCHM', 'Hauptstraße', 123, NULL, NULL, NULL, 1),
...
--
-- Stellvertreter-Struktur des Views `kurs_teilnehmer_dozent`
-- (Siehe unten für die tatsächliche Ansicht)
--
CREATE TABLE `kurs_teilnehmer_dozent` (
...
--
-- Struktur des Views `kurs_teilnehmer_dozent`
--
DROP TABLE IF EXISTS `kurs_teilnehmer_dozent`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `kurs_teilnehmer_dozent`  AS SELECT `k`.`Kursnummer` AS `Kurs_Kursnummer`, `k`.`Kurstyp` AS `Kurs_Kurstyp`, `k`.`Raum` AS `Kurs_Raum`, `k`.`Kursbeschreibung` AS `Kurs_Kursbeschreibung`, `k`.`KursBeginn` AS `Kurs_KursBeginn`, `k`.`KursEnde` AS `Kurs_KursEnde`, `d`.`Anrede` AS `Dozent_Anrede`, `d`.`Vorname` AS `Dozent_Vorname`, `d`.`Nachname` AS `Dozent_Nachname`, `d`.`Telefonnummer` AS `Dozent_Telefonnummer`, `d`.`Kürzel` AS `Dozent_Kürzel`, `t`.`Anrede` AS `Teilnehmer_Anrede`, `t`.`Vorname` AS `Teilnehmer_Vorname`, `t`.`Nachname` AS `Teilnehmer_Nachname`, `t`.`Email` AS `Teilnehmer_Email`, `t`.`Telefonnummer` AS `Teilnehmer_Telefonnummer` FROM ((((`kurs` `k` join `kurs_dozent` `kd` on(`k`.`ID_Kurs` = `kd`.`ID_Kurs`)) join `dozent` `d` on(`kd`.`ID_Dozent` = `d`.`ID_Dozent`)) join `kurs_teilnehmer` `kt` on(`k`.`ID_Kurs` = `kt`.`ID_Kurs`)) join `teilnehmer` `t` on(`kt`.`ID_Teilnehmer` = `t`.`ID_Teilnehmer`)) ;
...
--
ALTER TABLE `rechnung`
  MODIFY `ID_Rechnung` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=10;
